...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void initializeFeed() async {
    await Future.wait([
      'https://future-architect.github.io/atom.xml',
      'https://developer.apple.com/news/releases/rss/releases.rss'
    ].map((url) async => await fetchFeed(url).then((fi) => setState(() {
          _articles = _articles + [fi];
        }))));
  }
      ...
      drawer: Drawer(
        child: ListView(
          children: <Widget>[
            ...
            ..._articles.map((feedItem) => ExpansionTile(
                  title: Text(feedItem.title),
                  children: feedItem.items
                      .join(
                          (p0) => p0.map((article) =>
